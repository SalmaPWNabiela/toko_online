...
export default class Cart extends Component{
    ...
    removeFromCart = (products) => {
        ...
        let cart = carts.filter(item => item.id !== products.id);
        ...
        return(
            <div className="container">
                ...
                <h3 className="card-title">Cart 
                    <span className="float-right fa fa-cart-plus badge badge-secondary">
                        {num}
                    </span>
                </h3>
                <hr />
                        ...
                        <tbody>
                            {carts.map((products, index) =>
                                <tr key={index}>
                                    ...
                                    <td>
                                        <h5><span className="badge badge-secondary">
                                            Rp. {products.total}</span>
                                        </h5>
                                    </td>
                                    <td>
                                        <button className="btn btn-sm btn-warning"
                                         onClick={() => this.removeFromCart(products)}><span className="fa fa-trash">
                                             </span> Remove
                                        </button>
                                    </td>
                                </tr>
            ...
            { carts.length ?
                ...
                <Link to="/checkout">
                    <button className="btn btn-success float-right">
                        <span className="fa fa-check-circle"></span> Checkout
                    </button>
                </Link>
                    <button className="btn btn-danger float-right" onClick={this.clearCart}
                     style={{ marginRight: "10px" }}>
                         <span className="fa fa-trash"></span> Clear Cart
                     </button> <br /> <br /> <br />
            </div>
        );
    }
